...
Account * SignUp::account_log() const
{
	...
	if (u_a_choice == 'U')
	{
		account_ptr= new User(t_user_name, t_password, t_Adress, t_gender, t_phone, 0, 0);
		saveFile(account_ptr);
		save_Account_Only_File(account_ptr);
	}
	else if (u_a_choice == 'A')
	{
		account_ptr=new Admin(t_user_name, t_password, t_Adress, t_gender, t_phone, 1);
		saveFile(account_ptr);
		save_Account_Only_File(account_ptr);
	}
	return account_ptr;
...
{
	string fname = "database/"+obj->get_User_name() + ".txt";
	fstream outFile;
...
{
	string fname = "database/accounts.txt";
	fstream outFile;
	outFile.open(fname, ios::app);
	if (outFile)
	{
